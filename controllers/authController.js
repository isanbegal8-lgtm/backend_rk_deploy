...
exports.login = async (req, res) => {
  try {
    ...
    const token = jwt.sign(
      { id_user: user.id_user, role: user.role }, // Data dalam token
    ...
    res.json({
      ...
      data: {
        token: token, // <-- Penting! Token dikirim ke frontend
...
exports.createUser = async (req, res) => {
  try {
    ...
    const newUser = await User.create({ id_user, username, password, role });
...
exports.updateUser = async (req, res) => {
  try {
    ...
    const [updated] = await User.update({ password, role }, { where: { id_user: id } });
